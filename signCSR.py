...
def sign_csr(pubkey, csr, email, file_based=False):
    ...
    nonce_req = urllib.request("{0}/directory".format(CA))
    ...
    pub_hex, pub_exp = re.search(
        r"Modulus(?: \((?:2048|4096) bit\)|)\:\s+00:([a-f0-9\:\s]+?)Exponent\: ([0-9]+)",
    ...
    if not email:
        default_email = "admin@{0}".format(min(domains, key=len))
        stdout = sys.stdout
    ...
    reg_nonce = urllib.urlopen(nonce_req).headers['Replay-Nonce']
    reg_raw = json.dumps({
        "resource": "new-reg",
    ...
    for domain in domains:
        ...
        id_nonce = urllib.urlopen(nonce_req).headers['Replay-Nonce']
        id_raw = json.dumps({
            "resource": "new-authz",
            "identifier": {
                "type": "dns",
    ...
    csr_nonce = urllib.urlopen(nonce_req).headers['Replay-Nonce']
    csr_raw = json.dumps({
        "resource": "new-cert",
        "csr": csr_der64,
    }, sort_keys=True, indent=4)
...
""".format(
    ...
    try:
        resp = urllib.urlopen(reg_url, reg_data)
    ...
    for n, i in enumerate(ids):
        ...
        try:
            resp = urllib.urlopen(id_url, id_data)
            result = json.loads(resp.read())
        except urllib.HTTPError as e:
        ...
        test_nonce = urllib.urlopen(nonce_req).headers['Replay-Nonce']
        test_raw = json.dumps({
            "resource": "challenge",
...
""".format(n + 4, i['domain'], responses[n]['data']))
        ...
        try:
            resp = urllib.urlopen(test_url, test_data)
    ...
    try:
        resp = urllib.urlopen(csr_url, csr_data)
